...
TEST_CASE( "Test 1 factor/1 node gn solver", "[lls]") {
...
TEST_CASE( "Test 1 factor/1 node lm solver", "[lls]") {
    ::hitnlls::node::NodeVec2 nv2;
    ::hitnlls::matrix::Vector2f x_est;
    x_est[0] = 0.7;
    x_est[1] = 0.3;
    ...
    ::hitnlls::solver::LmSolver solver;
    solver.SetGraph(&graph);
    solver.Optimize();
    x_est = nv2.GetEstimate();
...
TEST_CASE( "Test 1 factor/1 node pcg solver", "[lls]") {
    ::hitnlls::node::NodeVec2 nv2;
    ::hitnlls::matrix::Vector2f x_est;
    x_est[0] = 0.7;
    x_est[1] = 0.3;
    ...
    ::hitnlls::solver::PcgSolver solver;
    solver.SetGraph(&graph);
    solver.Optimize();
    x_est = nv2.GetEstimate();
